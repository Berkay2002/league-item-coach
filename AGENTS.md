...
- Do not mark an issue implementation goal complete until its PR is merged into `main`. Copilot must review cleanly before merge; after follow-up fixes, request a Copilot re-review only when contextually needed, at most once, and stop for human decision if issues remain.
